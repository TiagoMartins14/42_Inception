...
if [ ! -f ./wp-config.php ]; then
    ...
    sleep 5
    php /usr/local/bin/wp/wp-cli.phar core install --path=$WP_HOME --url=$WP_SITEURL --title="tiafernaInception" --admin_user=$WP_ADMIN_USER --admin_password=$WP_ADMIN_PASSWORD --admin_email=$WP_ADMIN_EMAIL
